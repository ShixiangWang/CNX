...
From figures above we do clearly see that the `skmeans` partition method fits our data and any `top_value_method` is okay. Finally, we choose combine `skmeans` (partition method) and `ATC` (top value method introduced firstly by `cola`) to run clustering for all samples.
...
The report can be viewed at [here](./pcawg_abs_sigs_all_sampls_final/cola_report.html).
...
```
There are many details about all statistical measures shown above at [cola vignette](http://bioconductor.org/packages/devel/bioc/vignettes/cola/inst/doc/cola.html#toc_13). The best k `2` is suggested by `cola` package due to its better statistical measures. However, in this practice, I think `3` or `6` is more suitable. I have the following strong reasons.

The reasons for `k = 3`:

1. Measure `1 - PAC` reaches a local maximal at `k = 3` which indicates the subgroups are less due to ambiguously assigned.
2. Measure `area_increased` drops sharply at `k = 3` which indicates increasing `k` won't change the consensus matrix too much.
3. Measure `Jaccard` increases significantly at `k = 3` which indicate the subgroups at `k = 4` are highly similar as `k = 3`.

The reasons for `k = 6`:

1. Measure `1 - PAC` reaches top value at `k = 6`.
2. Measure `Jaccard` increases significantly at `k = 6` which indicate the subgroups at `k = 7` are highly similar as `k = 6`.
3. Measure `Silhouette` start dropping at `k = 6`.
...
```
We can see that subgroups for `k = 3, 4, 5` are similar and subgroups for `k = 6, 7, 8, 9, 10` are similar.
...
```{r, eval=FALSE}
pcawg_clusters <- get_classes(cluster_res, k = c(3, 6))
...
left_annotation = rowAnnotation(foo = anno_text(paste0("Sig", c(1:4, 6:11))))
```

Show heatmap for clusters based on signature activities.

```{r error=TRUE, fig.width=22, fig.height=10}
get_signatures(cluster_res, k = 6, silhouette_cutoff = 0.2, anno = anno_df,
...
get_signatures(cluster_res, k = 6, silhouette_cutoff = 0.2, anno = anno_df, left_annotation = left_annotation)
dev.off()
```

Considering the results of `6` clusters gives confident labels for `>90%` samples and thus there is no need to use `3` clusters in this study.
...
df_merged <- purrr::reduce(
  list(
    ...
    cluster_df[, -2] %>% purrr::set_names(c("sample", "cluster"))
  ),
  dplyr::left_join,
  by = "sample"
) %>% 
...
```{r}
enrich_result_cn$enrich_var <- factor(enrich_result_cn$enrich_var, paste0("Sig", 1:11))
...
```{r}
nm_id <- colnames(df_merged)[grepl("ID", colnames(df_merged))]
...
There are so many COSMIC signatures above, it is not easy to summaize the data. Considering all COSMIC signatures are labelled and many of them have been assigned to a specific etiology, here we try to use word cloud plot to summarize the result above.
...
#### Variant number and driver gene number comparison
...
enrich_result_mut <- group_enrichment(
  df_mut,
  grp_vars = "cluster",
  enrich_vars = c("Driver_variants", "Act_genes", "LoF_genes"),
  co_method = "wilcox.test"
)
```

```{r fig.height=4}
enrich_result_mut$enrich_var <- factor(enrich_result_mut$enrich_var, c("Driver_variants", "Act_genes", "LoF_genes"))
...
ComplexHeatmap::Heatmap(df_genes[, c(-1, -2)])
```
